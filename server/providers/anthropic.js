...
import { makeErrorEvent, mapAnthropicError, ERROR_CODES } from '../error-taxonomy.js';
    ...
    async start(controller) {
      ...
      watchdog = setInterval(() => {
        if (Date.now() - lastByteAt > IDLE_TIMEOUT_MS) {
          logger.warn('[anthropic] idle watchdog triggered');
      ...
      try {
        ...
        const stream = await client.messages.stream(streamParams, { signal: abortCtrl.signal });

        for await (const event of stream) {
          if (finished) break;
          if (event.type === 'content_block_delta' && event.delta?.type === 'text_delta') {
            const text = decoder.decode(Buffer.from(event.delta.text ?? ''), { stream: true });
            if (text) {
              lastByteAt = Date.now();
